...
use App\Services\HigherEducationAllowanceService;
use Illuminate\Support\Carbon;;
use Illuminate\Auth\Access\AuthorizationException;
use Ynotz\SmartPages\Http\Controllers\SmartController;

class HigherEducationController extends SmartController
{
    public function __construct(public HigherEducationAllowanceService $allowanceService, Request $request) {
    ...
    public function show($id)
    {
        ...
        if (auth()->user()->can('view', $application)) {
            return $this->buildResponse('admin.allowances.higher_education.show', ['application' => $application]);
        } else {
            return $this->buildResponse('admin.allowances.higher_education.show', ['error' => 'You are not authorised to view this receipt.', 'application' => null]);
        }
    }
    ...
    public function create()
    {
        ...
        if (auth()->user()->cannot('update', $member)) {
            return $this->buildResponse(
                'admin.allowances.higher_education.create',
                [
                    'member' => null,
                ]);
        ...
        $scheme = WelfareScheme::where('name', config('generalSettings.allowances')['higher_education'])->get()->first();
    ...
    public function edit($id)
    {
        ...
        return $this->buildResponse(
            'admin.allowances.higher_education.edit',
            [
                'allowance' => $allowance,
                'today' => $today
            ]);
