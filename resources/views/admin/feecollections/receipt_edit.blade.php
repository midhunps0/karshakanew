<x-easyadmin::partials.adminpanel>
    <h3 class="text-xl font-bold pb-3"><span>Edit Receipt</span>&nbsp;</h3>
    <div class="w-full text-right p-4 space-x-4">
    ...
    <div class="w-full"
        x-data="{
            ...
            doSubmit() {
                ...
                $dispatch('formsubmit', { url: this.postUrl.replace('_X_', this.receipt_id), formData: fd, target: '{{$form['id']}}', fragment: 'create_form' });
            }
        }"
        x-init="
        @if (isset($receipt))
            ...
            @foreach ($receipt->feeItems as $fi)
                fees.push({
                        ...
                        from: null,
                        @endif
                        @if (isset($fi->formatted_period_to))
                        to: '{{$fi->formatted_period_to}}',
                        @else
                        to: null,
                        @endif
                        amount: {{$fi->amount}},
        ...
        <div x-show="member != null">
            <h3 class="text-sm font-bold pb-3 text-warning">Receipt For:</h3>
        ...
        <form x-show="showform"
            ...
            @formresponse.window="console.log($event.detail);
                ...
                console.log($event.detail.content.receipt);
                        ...
                        $dispatch('shownotice', {message: 'Receipt Updated', mode: 'success', });
            ...
            id="{{$form['id']}}"
            x-init="
            postUrl='{{route('feecollections.update', '_X_')}}'
                    ...
                    <div class="p-4 border border-base-content border-opacity-20 rounded-md bg-base-200">
                        <div>
                            ...
                            <div class="flex flex-wrap space-x-0 md:space-x-4 items-end">
                                ...
                                <button class="my-2 md:my-0 btn btn-md btn-warning flex-grow"> Update </button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div id="receipt" x-show="showreceipt" class="w-full md:w-10/12 m-auto p-3 border            border-base-content border-opacity-50 rounded-md my-8 min-w-48 overflow-x-scroll">
            ...
            <div class="flex flex-row space-x-4 justify-center items-center p-4 mt-4 print:hidden">
                ...
                <button @click.prevent.stop="$dispatch('linkaction', {link: '{{route('dashboard')}}', route: 'dashboard'})" class="btn btn-sm btn-error">Close</button>
            </div>
        </div>
